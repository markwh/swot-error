...
load.project()
valdata <- rt_valdata("output/sac05", group = "nodes") %>% 
...
This is a copy of the previous slides just with the updated prior db from Rui. 
...
- RiverObs gives measurements with $1 \sigma$ uncertainty for
    ...
    - lat, lon
    - area (total)
- All this at node scale (264 nodes)
- Single overpass

## Goal: validate uncertainty model

```{r}
include_graphics("https://imgs.xkcd.com/comics/error_bars.png")
```

- We have error bars ($1 \sigma$ uncertainty), but they're theoretically derived
- How well do they match empirical errors?
...
pcvdata <- pixcvec_read("output/sac05/pcv.nc") %>% 
  dplyr::filter(node_index %in% badnodes) %>% 
  rename(lat = latitude_vectorproc, lon = longitude_vectorproc)

pcvdata_gdem <- pixcvec_read("output/sac05/pcv_gdem.nc") %>% 
  dplyr::filter(node_index %in% badnodes) %>% 
  rename(lat = latitude_vectorproc, lon = longitude_vectorproc)
