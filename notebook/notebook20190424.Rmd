...
connect4 <- function(pcdf) {
  ...
  advance_front_ds <- function(front) {
    ...
    starti <- 1
    for (index in front) {
      new_nbrs <- dnstream_neighbors(index, nn = nn)
  ...
  advance_front_us <- function(front) {
    ...
    starti <- 1
    for (index in front) {
      new_nbrs <- upstream_neighbors(index, nn = nn)
      if (!(n_new_nbrs <- length(new_nbrs))) next
      endi <- starti + n_new_nbrs - 1
      new_front[starti:endi] <- new_nbrs
      starti <- endi + 1
  ...
  # for (i in 1:300) {
    ...
    count_ds <- count_ds + 1
    if (count_ds %% 100 == 0) cat(sum(connected_ds))
  }
  ...
  # for (i in 1:3) {
    ...
    count_us <- count_us + 1
    if (count_us %% 100 == 0) cat(sum(connected_us))
  }
  message("Upstream connection complete")
