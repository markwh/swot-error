...
# ncloc <- "~/Documents/swot-error/data/priordb_update/Sac_sample_db14.nc"
...
mapdf <- testdf %>% 
  ...
  select(latitude, longitude, con_no, node_index, reach_index, pixc_index, 
         radius, along, frontno_ds) %>% 
  filter(reach_index == 5) %>% 
  # sample_n(10000) %>% 
  glimpse()


conpal <- colorFactor("Set1", domain = mapdf$con_no)
...
gg4 <- ggplot(mapdf, aes(y = longitude, x = latitude)) +
  geom_point(size = 0.2) +
  geom_point(size = 0.2, aes(group = frontno_ds), 
...
gg4 + transition_time(frontno_ds) +
  shadow_mark()
```
